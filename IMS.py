...
class IMS:
    ...
    def add_item(self, home_page):
        item_name = input("Enter item name: ")
        quantity = home_page.get_positive_number("Enter quantity: ")
        price = home_page.get_positive_number("Enter price: ")
    ...
    def edit_item(self, home_page):
        item_name = input("Enter item name to edit: ")
        self.cursor.execute("SELECT * FROM inventory WHERE item_name = ?", (item_name,))
        item = self.cursor.fetchone()
        if item:
            new_quantity = home_page.get_positive_number("Enter new quantity: ")
            new_price = home_page.get_positive_number("Enter new price: ")
            self.cursor.execute(
...
class HomePage:
    def __init__(self):
        ...
        self.inventory_system = IMS(self.db_name)
    ...
    def main(self):
        while True:
            ...
            if choice == "1":
                self.inventory_system.add_item(self)
            elif choice == "2":
                self.inventory_system.delete_item()
            elif choice == "3":
                self.inventory_system.search_item()
            elif choice == "4":
                self.inventory_system.edit_item(self)
            elif choice == "5":
                self.inventory_system.display_inventory()
            elif choice == "6":
                self.inventory_system.close_connection()
                break
            else:
